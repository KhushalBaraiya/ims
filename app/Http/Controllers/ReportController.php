...
class ReportController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $summary = [
            ...
            'low_stock'       => Product::whereHas('stock', fn($q) =>
                                    $q->where('quantity', '>', 0)
                                      ->whereRaw('stocks.quantity <= products.minimum_stock_alert')
                                  )->count(),
            'out_of_stock'    => Product::where(fn($q) =>
    ...
    public function profitLoss(Request $request): View
    {
        ...
        $totalCogs = $saleItems->sum(fn($item) =>
            ($item->product->purchase_price ?? 0) * $item->quantity
        );
        ...
        if ($filter === 'out') {
            ...
            $query->where(fn($q) =>
                $q->whereHas('stock', fn($sq) =>
                    $sq->whereRaw('stocks.quantity <= products.minimum_stock_alert')
                  )
                  ->orWhereDoesntHave('stock')
            );
        }
