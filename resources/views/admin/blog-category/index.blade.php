...
@section('content')
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="mb-0 fw-semibold fs-4">{{ __('admin.blog_category_list') }}</h4>
            <a href="{{ route('admin.blog-category.create') }}" class="btn btn-primary btn-lg">
                <i class="bx bx-plus"></i> {{ __('admin.add') }} {{ __('admin.blog_categories') }}
            </a>
        </div>
        <div class="table-responsive p-3">
            <table class="table table-hover align-middle mb-0 fs-5" id="blogCatTable">
                <thead>
                    <tr>
                        <th>{{ __('admin.id') }}</th>
                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td>{{ Str::limit($category->description, 60) ?? __('admin.dash') }}</td>
                            <td><span class="badge bg-info rounded-pill px-3 py-1">{{ $category->blogs_count }}</span></td>
                            <td class="text-center">
                                <button
                                    ...
                                    data-id="{{ $category->id }}" data-model="BlogCategory">
