...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-1">{{ isset($cartItem) ? __('admin.edit_cart_item') : __('admin.add_cart_item') }}</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 fs-6">
                    ...
                    <li class="breadcrumb-item active">{{ isset($cartItem) ? __('admin.edit') : __('admin.add') }}</li>
                </ol>
            </nav>
        </div>
    ...
    <form id="cartItemForm"
        action="{{ isset($cartItem) ? route('admin.cart-item.update', $cartItem->id) : route('admin.cart-item.store') }}"
        method="POST">
        @csrf
        @if (isset($cartItem))
            @method('PUT')
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h6 class="mb-0 fw-semibold fs-5"><i
                                class="bx bx-cart-alt me-2 text-primary"></i>{{ __('admin.cart_item_details') }}
                        </h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <div class="select2-lg">
                                <select name="user_id" class="select2-user form-select form-select-lg w-100">
                                    <option value=""
                                        {{ old('user_id', $cartItem->user_id ?? '') == '' ? 'selected' : '' }}>
                                        {{ __('admin.select_user') }}
                                    </option>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ old('user_id', $cartItem->user_id ?? '') == $user->id ? 'selected' : '' }}>
                                            {{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="select2-lg">
                                <select name="product_id" id="productSelect"
                                    ...
                                    <option value=""
                                        {{ old('product_id', $cartItem->product_id ?? '') == '' ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="quantity" class="form-control form-control-lg" min="1"
                                placeholder="{{ __('admin.qty_placeholder') }}"
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="form-check form-switch mb-0">
                                    <input class="form-check-input toggle-switch-input" type="checkbox" role="switch"
                                        ...
                                        {{ old('status', $cartItem->status ?? 'active') == 'active' ? 'checked' : '' }}>
                                </div>
                                <span id="statusLabel"
                                    class="fw-semibold fs-6 {{ old('status', $cartItem->status ?? 'active') == 'active' ? 'text-success' : 'text-danger' }}">
                                    {{ old('status', $cartItem->status ?? 'active') == 'active' ? __('admin.status_active') : __('admin.status_inactive') }}
                                </span>
                            </div>
                        </div>
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg"><i class="bx bx-save me-1"></i>
                                {{ isset($cartItem) ? __('admin.update') : __('admin.save') }}</button>
                            <a href="{{ route('admin.cart-item.index') }}" class="btn btn-outline-secondary btn-lg"><i
