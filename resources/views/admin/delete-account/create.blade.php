...
@section('title', isset($deleteAccount) ? __('admin.edit_record') : __('admin.add_delete_account_reason'))
@section('content')

    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-1">
                {{ isset($deleteAccount) ? __('admin.edit_record') : __('admin.add_delete_account_reason') }}</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 fs-6">
                    <li class="breadcrumb-item"><a
                            href="{{ route('admin.dashboard') }}">{{ __('admin.breadcrumb_dashboard') }}</a></li>
                    <li class="breadcrumb-item"><a
                            href="{{ route('admin.delete-account.index') }}">{{ __('admin.delete_account_list') }}</a></li>
                    <li class="breadcrumb-item active">{{ isset($deleteAccount) ? __('admin.edit') : __('admin.add') }}</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('admin.delete-account.index') }}" class="btn btn-outline-secondary btn-lg">
    ...
    <form id="deleteAccountForm"
        action="{{ isset($deleteAccount) ? route('admin.delete-account.update', $deleteAccount) : route('admin.delete-account.store') }}"
        method="POST">
        @csrf
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h6 class="mb-0 fw-semibold fs-5"><i
                                class="bx bx-user-x me-2 text-primary"></i>{{ __('admin.details') }}</h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title"
                                ...
                                placeholder="{{ __('admin.ph_delete_account_title') }}"
                                value="{{ old('title', $deleteAccount->title ?? '') }}">
                            @error('title')
                    ...
                    <div class="card-body p-4">
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="form-check form-switch mb-0">
                                    <input class="form-check-input toggle-switch-input" type="checkbox" role="switch"
                                        ...
                                        {{ old('status', $deleteAccount->status ?? 'active') == 'active' ? 'checked' : '' }}>
                                </div>
                                <span id="statusLabel"
                                    class="fw-semibold fs-6 {{ old('status', $deleteAccount->status ?? 'active') == 'active' ? 'text-success' : 'text-danger' }}">
                                    {{ old('status', $deleteAccount->status ?? 'active') == 'active' ? __('admin.status_active') : __('admin.status_inactive') }}
                                </span>
                            </div>
                        </div>
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg">
                                ...
                                {{ isset($deleteAccount) ? __('admin.update') : __('admin.save') }}
                            </button>
                            <a href="{{ route('admin.delete-account.index') }}" class="btn btn-outline-secondary btn-lg"><i
