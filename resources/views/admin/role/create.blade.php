...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-1">{{ isset($role) ? __('admin.edit_role') : __('admin.add_role') }}</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 fs-6">
                    ...
                    <li class="breadcrumb-item active">{{ isset($role) ? __('admin.edit') : __('admin.add') }}</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('admin.role.index') }}" class="btn btn-outline-secondary btn-lg">
    ...
    <form id="roleForm" action="{{ isset($role) ? route('admin.role.update', $role->id) : route('admin.role.store') }}"
        method="POST">
        @csrf
        @if (isset($role))
            @method('PUT')
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h6 class="mb-0 fw-semibold fs-5"><i
                                class="bx bx-shield me-2 text-primary"></i>{{ __('admin.role_details') }}</h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">{{ __('admin.role_name') }} <span
                                    class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control form-control-lg"
                                placeholder="{{ __('admin.ph_role_name') }}" value="{{ old('name', $role->name ?? '') }}">
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="form-check form-switch mb-0">
                                    <input class="form-check-input toggle-switch-input" type="checkbox" role="switch"
                                        ...
                                        {{ old('status', $role->status ?? 'active') == 'active' ? 'checked' : '' }}>
                                </div>
                                <span id="statusLabel"
                                    class="fw-semibold fs-6 {{ old('status', $role->status ?? 'active') == 'active' ? 'text-success' : 'text-danger' }}">
                                    {{ old('status', $role->status ?? 'active') == 'active' ? __('admin.status_active') : __('admin.status_inactive') }}
                                </span>
                            </div>
                        </div>
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg"><i class="bx bx-save me-1"></i>
                                {{ isset($role) ? __('admin.update') : __('admin.save') }}</button>
    ...
    <script>
        $(document).ready(function() {
            $('#roleForm').validate({
                ...
                messages: {
                    name: {
                        required: "{{ __('admin.val_role_name_required') }}",
