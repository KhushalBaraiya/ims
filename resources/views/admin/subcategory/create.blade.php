...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-1">{{ isset($subCategory) ? __('admin.edit_sub_category') : __('admin.add_sub_category') }}
            </h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 fs-6">
                    ...
                    <li class="breadcrumb-item"><a
                            href="{{ route('admin.subcategory.index') }}">{{ __('admin.sub_category_list') }}</a></li>
                    <li class="breadcrumb-item active">{{ isset($subCategory) ? __('admin.edit') : __('admin.add') }}</li>
                </ol>
            </nav>
        </div>
    ...
    <form id="subCategoryForm"
        action="{{ isset($subCategory) ? route('admin.subcategory.update', $subCategory->id) : route('admin.subcategory.store') }}"
        method="POST">
        @csrf
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h6 class="mb-0 fw-semibold fs-5"><i
                                class="bx bx-category me-2 text-primary"></i>{{ __('admin.sub_category_details') }}</h6>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">{{ __('admin.sub_category_name') }} <span
                                    class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control form-control-lg"
                                placeholder="{{ __('admin.ph_subcategory_name') }}"
                    ...
                    <div class="card-body p-4">
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="form-check form-switch mb-0">
                                    <input class="form-check-input toggle-switch-input" type="checkbox" role="switch"
                                        ...
                                        {{ old('status', $subCategory->status ?? 'active') == 'active' ? 'checked' : '' }}>
                                </div>
                                <span id="statusLabel"
                                    class="fw-semibold fs-6 {{ old('status', $subCategory->status ?? 'active') == 'active' ? 'text-success' : 'text-danger' }}">
                                    {{ old('status', $subCategory->status ?? 'active') == 'active' ? __('admin.status_active') : __('admin.status_inactive') }}
                                </span>
                            </div>
                        </div>
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg">
                                ...
                                {{ isset($subCategory) ? __('admin.update') : __('admin.save') }}
                            </button>
                            <a href="{{ route('admin.subcategory.index') }}" class="btn btn-outline-secondary btn-lg"><i
        ...
        $(document).ready(function() {
            ...
            $('#subCategoryForm').validate({
                ...
                messages: {
                    ...
                    name: {
                        required: "{{ __('admin.val_subcategory_name_required') }}",
