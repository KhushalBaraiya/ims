...
@section('content')
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="mb-0 fw-semibold fs-4">{{ __('admin.wishlist_list') }}</h4>
            <a href="{{ route('admin.wishlist.create') }}" class="btn btn-primary btn-lg">
                <i class="bx bx-plus"></i> {{ __('admin.add_wishlist') }}
            </a>
        </div>
        <div class="table-responsive p-3">
            <table class="table table-hover align-middle mb-0 fs-5" id="wishlistTable">
                <thead>
                    <tr>
                        <th>{{ __('admin.id') }}</th>
                        <th>{{ __('admin.user') }}</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <strong>{{ $wishlist->user->name ?? __('admin.na') }}</strong>
                                </div>
                            </td>
                            <td>{{ $wishlist->product->name ?? __('admin.na') }}</td>
                            <td>{{ $wishlist->created_at->format('d M Y') }}</td>
                            <td class="text-center">
                                <button
                                    class="status-toggle-btn badge rounded-pill border px-3 py-2 bg-transparent {{ $wishlist->status === 'active' ? 'border-success text-success' : 'border-danger text-danger' }}"
                                    data-id="{{ $wishlist->id }}" data-model="WishList">
                                    {{ $wishlist->status === 'active' ? __('admin.active') : __('admin.inactive') }}
                                </button>
                            </td>
                            <td class="text-center">
                                <a href="{{ route('admin.wishlist.show', $wishlist->id) }}"
