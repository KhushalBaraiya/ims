...
@section('content')
    ...
    <form method="POST" action="{{ route('password.email') }}" class="space-y-5">
        ...
        <button type="submit"
            ...
            <i class="fa-solid fa-paper-plane"></i> {{ __('messages.send_reset_link') }}
        </button>

        <div class="text-center mt-6">
            <a href="{{ route('login') }}"
                class="text-xs text-zinc-400 hover:text-zinc-200 transition-colors inline-flex items-center gap-2">
                <i class="fa-solid fa-arrow-left"></i> {{ __('messages.back_to_sign_in') }}
            </a>
        </div>
    </form>
@endsection
