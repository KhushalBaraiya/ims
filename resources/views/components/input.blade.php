...
<div>
    ...
    <div class="relative rounded-lg shadow-sm">
        <input type="{{ $type }}" name="{{ $name }}" id="{{ $name }}" value="{{ $value }}" placeholder="{{ $placeholder }}" {{ $required ? 'required' : '' }}
            {{ $attributes->merge(['class' => 'block w-full rounded-lg border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-950 py-2 px-3.5 text-[14px] text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-600 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 ' . ($errors->has($name) ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30' : '')]) }}>
    </div>
    @error($name)
