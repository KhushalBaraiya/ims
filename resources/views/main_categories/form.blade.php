...
<div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
    ...
    <div class="sm:col-span-3">
        <label for="name" class="block text-sm font-semibold text-slate-700">Category Name <span class="text-red-500">*</span></label>
        <div class="mt-1">
            <input type="text" name="name" id="name" value="{{ old('name', $mainCategory->name ?? '') }}" required
                ...
                placeholder="e.g. Smart TV, Smartphone, Laptop">
        </div>
        @error('name')
    ...
    <!-- Category Code (slug) -->
    <div class="sm:col-span-3">
        <label for="slug" class="block text-sm font-semibold text-slate-700">Category Code <span class="text-red-500">*</span></label>
        <div class="mt-1">
            <input type="text" name="slug" id="slug" value="{{ old('slug', $mainCategory->slug ?? '') }}" required
                ...
                placeholder="e.g. SMART-TV, SMARTPHONE">
        </div>
        <p class="mt-1 text-xs text-slate-400">Unique alphanumeric code used to identify the category.</p>
        @error('slug')
            <p class="mt-1.5 text-xs text-red-500 flex items-center gap-1">
                <i class="fa-solid fa-circle-info"></i> {{ $message }}
            </p>
    ...
    <div class="sm:col-span-6">
        ...
        <div class="mt-1">
            <textarea id="description" name="description" rows="3"
                ...
                placeholder="Describe this category briefly...">{{ old('description', $mainCategory->description ?? '') }}</textarea>
        </div>
        @error('description')
            <p class="mt-1.5 text-xs text-red-500 flex items-center gap-1">
                <i class="fa-solid fa-circle-info"></i> {{ $message }}
    ...
    <div class="sm:col-span-3">
        ...
        <div class="mt-1">
            <select id="status" name="status" required
                ...
                <option value="active" {{ old('status', $mainCategory->status ?? 'active') === 'active' ? 'selected' : '' }}>Active</option>
                <option value="inactive" {{ old('status', $mainCategory->status ?? '') === 'inactive' ? 'selected' : '' }}>Inactive</option>
            </select>
        </div>
        @error('status')
            <p class="mt-1.5 text-xs text-red-500 flex items-center gap-1">
...
<div class="mt-8 pt-5 border-t border-slate-100 flex justify-end gap-3">
    <a href="{{ route('main-categories.index') }}" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
        Cancel
    </a>
    <button type="submit" class="rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 px-5 py-2 text-sm font-semibold text-white shadow-md hover:from-blue-500 hover:to-violet-500 transition-all active:scale-[0.98]">
        {{ isset($mainCategory) ? 'Update Category' : 'Save Category' }}
    </button>
</div>
