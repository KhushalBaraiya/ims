    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="card shadow-sm mb-4">
                <div
                    ...
                    <span class="badge rounded-pill {{ $mainCategory->status === 'active' ? 'bg-success' : 'bg-danger' }}">
                        {{ $mainCategory->status === 'active' ? __('messages.active') : __('messages.inactive') }}
                    </span>
                </div>
                <div class="card-body p-4">
                    <div class="d-flex align-items-center gap-4 mb-4">
                        ...
                        <div>
                            <h4 class="fw-bold mb-1">{{ $mainCategory->name }}</h4>
                            <code class="text-primary">{{ $mainCategory->slug }}</code>
                            <p class="text-muted small mb-0 mt-1">
                                <i class="bx bx-calendar me-1"></i>
                                {{ __('messages.th_created') }}: {{ $mainCategory->created_at->format('d M Y, h:i A') }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-4">
                            <div class="rounded-3 p-3 text-center bg-label-primary">
                                <div class="fw-bold fs-4 text-primary">{{ $mainCategory->products->count() }}</div>
                                <div class="text-muted small">{{ __('messages.total_products') }}</div>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="rounded-3 p-3 text-center bg-label-success">
                                <div class="fw-bold fs-4 text-success">{{ $mainCategory->subCategories->count() }}</div>
                                <div class="text-muted small">{{ __('messages.sub_categories') }}</div>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="rounded-3 p-3 text-center bg-label-warning">
                                <div class="fw-bold fs-4 text-warning">
                                    {{ $mainCategory->products->where('sub_category_id', '!=', null)->count() }}
                                </div>
                                <div class="text-muted small">{{ __('messages.sub_in_category') }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($mainCategory->description)
                        <div class="border-top pt-3">
                            ...
                            <p class="mb-0">{{ $mainCategory->description }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card shadow-sm mb-4">
                <div
                    ...
                    @can('sub_categories.create')
                        <a href="{{ route('sub-categories.create') }}?main_category_id={{ $mainCategory->id }}"
                            class="btn btn-sm btn-outline-success">
                            <i class="bx bx-plus me-1"></i>{{ __('messages.add') }}
                        </a>
                    @endcan
                </div>
                    ...
                    @if ($mainCategory->subCategories->count())
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    @foreach ($mainCategory->subCategories as $i => $sub)
                                        <tr>
                                            <td class="ps-4 text-muted">{{ $i + 1 }}</td>
                                            <td class="fw-semibold">{{ $sub->name }}</td>
                                            <td><code class="text-primary small">{{ $sub->slug }}</code></td>
                                            <td class="text-center">
                                                <span
                                                    class="badge rounded-pill {{ $sub->status === 'active' ? 'bg-success' : 'bg-danger' }}">
                                                    {{ $sub->status === 'active' ? __('messages.active') : __('messages.inactive') }}
                                                </span>
                                            </td>
                                            <td class="text-center">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body p-0">
                    @php $recentProducts = $mainCategory->products->sortByDesc('created_at')->take(5); @endphp
                                        ...
                                        <tr>
                                            <td class="ps-4">
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    @else
                                                        <div
                                                            class="tbl-img img-fallback tbl-img d-flex align-items-center justify-content-center bg-light rounded">
        ...
        <div class="col-lg-4">
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body p-4">
                    <ul class="list-unstyled mb-0">
                        ...
                        <li class="d-flex justify-content-between py-2 border-bottom">
                            <span class="text-muted small fw-semibold">{{ __('messages.category_name') }}</span>
                            <span class="fw-bold">{{ $mainCategory->name }}</span>
                        </li>
                        <li class="d-flex justify-content-between py-2 border-bottom">
                            <span class="text-muted small fw-semibold">{{ __('messages.th_status') }}</span>
                            <span
                                class="badge rounded-pill {{ $mainCategory->status === 'active' ? 'bg-success' : 'bg-danger' }}">
                                {{ $mainCategory->status === 'active' ? __('messages.active') : __('messages.inactive') }}
                            </span>
                        </li>
                        <li class="d-flex justify-content-between py-2 border-bottom">
                            <span class="text-muted small fw-semibold">{{ __('messages.total_products') }}</span>
                            <span class="badge bg-label-info">{{ $mainCategory->products->count() }}</span>
                        </li>
                        <li class="d-flex justify-content-between py-2 border-bottom">
                            <span class="text-muted small fw-semibold">{{ __('messages.sub_categories') }}</span>
                            <span class="badge bg-label-success">{{ $mainCategory->subCategories->count() }}</span>
                        </li>
                        <li class="d-flex justify-content-between py-2 border-bottom">
                            <span class="text-muted small fw-semibold">{{ __('messages.th_created') }}</span>
                            <span class="small">{{ $mainCategory->created_at->format('d M Y') }}</span>
                        </li>
                        <li class="d-flex justify-content-between py-2">
                            <span class="text-muted small fw-semibold">{{ __('messages.updated') }}</span>
                            <span class="small">{{ $mainCategory->updated_at->format('d M Y') }}</span>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body p-4 d-grid gap-2">
                    @can('main_categories.update')
                        <a href="{{ route('main-categories.edit', $mainCategory->id) }}" class="btn btn-primary">
                            <i class="bx bx-edit me-1"></i> {{ __('messages.edit_category') }}
                        </a>
                    @endcan
                    @can('sub_categories.create')
                        <a href="{{ route('sub-categories.create') }}?main_category_id={{ $mainCategory->id }}"
                    ...
                    @can('main_categories.delete')
                        <form id="deleteForm" action="{{ route('main-categories.destroy', $mainCategory->id) }}"
                            ...
                            <button type="button" class="btn btn-outline-danger w-100 delete-btn"
                                ...
                                <i class="bx bx-trash me-1"></i> {{ __('messages.delete_category') }}
                            </button>
                        </form>
                    @endcan
                </div>
