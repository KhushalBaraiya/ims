            ...
            <div class="col-lg-8">
                ...
                <div class="card shadow-sm mb-4">
                    <div
                        ...
                        <h6 class="mb-0 fw-semibold">
                            <i class="bx bx-key me-2 text-warning"></i>{{ __('messages.module_actions_selection') }}
                        </h6>
                        <div class="form-check form-switch mb-0">
                            ...
                            <label class="form-check-label fw-semibold small"
                                for="selectAllActionsSwitch">{{ __('messages.select_all_actions') }}</label>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <div class="alert alert-primary d-flex align-items-center gap-2 mb-4 py-2">
                            ...
                            <div>
                                <span class="fw-semibold">{{ __('messages.generated_permissions') }}</span>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="moduleInput" name="module" list="moduleList"
                                ...
                                placeholder="{{ __('messages.ph_permission_module') }}" value="{{ old('module') }}"
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">
                                {{ __('messages.select_actions_to_create') }} <span class="text-danger">*</span>
                            </label>
                            <div class="row g-3">
                                @php
                                    $standardActions = [
                                            ...
                                            <div class="card-body p-3 d-flex align-items-center gap-2">
                                                <div class="form-check mb-0">
                                                    <input class="form-check-input action-checkbox" type="checkbox"
                                                        name="actions[]" value="{{ $actKey }}"
                                                        id="act_{{ $actKey }}" @checked(in_array($actKey, old('actions', ['view', 'create', 'update', 'delete'])))>
                                                </div>
                                                <div>
                                                    <label class="form-check-label fw-semibold mb-0"
                                                        for="act_{{ $actKey }}">
                                                        <span
                                                            class="badge {{ $actMeta['badge'] }} me-1">{{ ucfirst($actKey) }}</span>
                                                    </label>
                        ...
                        <div class="mt-4 pt-3 border-top">
                            <label
                                class="form-label fw-semibold small text-muted">{{ __('messages.or_add_custom_action') }}</label>
                            <div class="input-group input-group-merge style-input-group">
                                ...
                                <input type="text" id="customActionInput" name="custom_action" class="form-control"
                                    placeholder="{{ __('messages.ph_permission_action') }}"
                                    value="{{ old('custom_action') }}">
                            </div>
                            <div class="form-text">{{ __('messages.custom_action_batch_hint') }}</div>
                        </div>
                ...
                <div class="card shadow-sm">
                    <div
                        ...
                        <div>
                            <h6 class="mb-0 fw-semibold">
                                <i class="bx bx-shield me-2 text-primary"></i>{{ __('messages.assign_to_roles_title') }}
                            </h6>
                            <small class="text-muted">{{ __('messages.assign_to_roles_sub') }}</small>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            <button type="button" class="btn btn-xs btn-outline-primary" id="presetSuperAdminAll">
                                <i class="bx bx-crown me-1"></i>{{ __('messages.super_admin_all_btn') }}
                            </button>
                            <button type="button" class="btn btn-xs btn-outline-secondary" id="presetClearAll">
                                {{ __('messages.clear_all_btn') }}
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle mb-0">
                                <thead class="table-light">
                                    <tr>
                                        <th style="min-width:140px;">{{ __('messages.role_name_col') }}</th>
                                        <th class="text-center" style="width:70px;">All</th>
                                        @foreach (['view', 'create', 'update', 'delete', 'own'] as $actHeader)
                                            <th class="text-center text-capitalize action-col-header"
                                                data-action="{{ $actHeader }}" style="width:80px;">
                                                {{ $actHeader }}
                                            </th>
                                        @endforeach
                                    </tr>
                                    ...
                                    @foreach ($allRoles as $role)
                                        ...
                                        <tr data-role-id="{{ $role->id }}">
                                            <td class="fw-semibold">
                                                @if ($isSuper)
                                                    <span class="badge bg-label-warning me-1"><i class="bx bx-crown"></i>
                                                        {{ $role->name }}</span>
                                                @elseif ($isManager)
                                                    <span class="badge bg-label-primary me-1"><i class="bx bx-shield"></i>
                                                        {{ $role->name }}</span>
                                                @else
                                                    <span class="badge bg-label-secondary me-1"><i class="bx bx-user"></i>
                                                        {{ $role->name }}</span>
                                                @endif
                                            </td>
                                            <td class="text-center">
                                                <div class="form-check d-flex justify-content-center mb-0">
                                                    <input class="form-check-input role-row-all-switch" type="checkbox"
                                                        ...
                                                        title="Toggle all actions for {{ $role->name }}"
                                                        @checked($isSuper)>
                                                </div>
                                            </td>
                                            @foreach (['view', 'create', 'update', 'delete', 'own'] as $act)
                                                @php
                                                    ...
                                                    $defaultChecked =
                                                        $isSuper ||
                                                        ($isManager && in_array($act, ['view', 'create', 'update'])) ||
                                                        (!$isSuper && !$isManager && $act === 'view');
                                                @endphp
                                                <td class="text-center">
                                                    <div class="form-check d-flex justify-content-center mb-0">
                                                        <input class="form-check-input role-action-checkbox"
                                                            type="checkbox" name="role_actions[{{ $role->id }}][]"
                                                            value="{{ $act }}"
                                                            data-role-id="{{ $role->id }}"
                                                            data-action="{{ $act }}"
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm sticky-top" style="top:90px;">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h6 class="mb-0 fw-semibold">
                            <i class="bx bx-send me-2 text-primary"></i>{{ __('messages.publish_save_title') }}
                        </h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="alert alert-success d-flex align-items-center gap-2 mb-4 py-2">
                            ...
                            <div class="small">
                                Created permissions will be assigned to selected roles <strong>instantly</strong> without
                                running seeders.
                            </div>
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg">
                                <i class="bx bx-save me-1"></i> {{ __('messages.save_permissions_roles') }}
                            </button>
                            <a href="{{ route('permissions.index') }}" class="btn btn-outline-secondary">
        ...
        (function() {
            ...
            function updatePreview() {
                ...
                if (selectedActions.length === 0) {
                    previewBadgeList.innerHTML =
                        `<code class="bg-white px-2 py-1 rounded text-muted border">${module}.action</code>`;
                    return;
                }
                ...
                selectedActions.forEach(act => {
                    html +=
                        `<code class="bg-white px-2 py-1 rounded text-primary border me-1 mb-1">${module}.${act}</code>`;
                });
            ...
            document.querySelectorAll('.role-row-all-switch').forEach(rowSwitch => {
                rowSwitch.addEventListener('change', function() {
                    ...
                    document.querySelectorAll(`.role-action-checkbox[data-role-id="${roleId}"]`)
                        .forEach(chk => {
                            if (!chk.disabled) {
                                chk.checked = checked;
                            }
            ...
            document.querySelectorAll('.role-action-checkbox').forEach(chk => {
                chk.addEventListener('change', function() {
                    const roleId = this.dataset.roleId;
                    const roleChecks = Array.from(document.querySelectorAll(
                        `.role-action-checkbox[data-role-id="${roleId}"]:not(:disabled)`));
                    const rowSwitch = document.querySelector(
                        `.role-row-all-switch[data-role-id="${roleId}"]`);
            ...
            if (presetSuperAdminAll) {
                presetSuperAdminAll.addEventListener('click', function() {
                    const superRowSwitch = document.querySelector(
                        'tr[data-role-id="1"] .role-row-all-switch') || document.querySelector(
                        '.role-row-all-switch');
                    if (superRowSwitch) {
