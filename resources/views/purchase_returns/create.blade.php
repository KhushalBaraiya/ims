                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div id="noItemsMsg" class="text-center py-5 text-muted">
                            <i class="bx bx-search-alt" style="font-size:2.5rem;opacity:.3;"></i>
                            <p class="mt-2 mb-0">Search and add products to return above.</p>
                        </div>
...
$(document).ready(function () {
    ...
    function loadPurchaseItems(callback) {
        $.ajax({
            ...
            error: function () {
                noItemsMsg.html('<p class="text-danger">Failed to load purchase items.</p>').removeClass('d-none');
            }
        });
    }
    ...
    function renderSearchResults(items, mode) {
        ...
        items.forEach(item => {
            ...
            if (mode === 'purchase') {
                ...
                badge = `<div class="text-muted" style="font-size:11px;">Stock: ${parseFloat(stock).toFixed(0)}</div>`;
            }
    ...
    $(document).on('click', '.autocomplete-item', function () {
        ...
        if (mode === 'free') {
            ...
            $.ajax({
                url: '{{ route("purchase-returns.search-products") }}',
                type: 'GET',
                data: { query: id },  // won't match by id, so re-search
                success: function () {}
            });
        ...
        @foreach (old('items', []) as $oldIndex => $oldItem)
            @if (!empty($oldItem['product_id']))
                ...
                @if ($oldProduct)
                    ...
                    }, {{ (int)($oldItem['quantity'] ?? 1) }}, "{{ addslashes($oldItem['reason'] ?? '') }}", false);
                @endif
            @endif
        @endforeach
    @endif
