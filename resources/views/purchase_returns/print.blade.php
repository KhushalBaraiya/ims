...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen py-10 print:py-0 print:bg-white print:text-black">
    ...
    <div class="max-w-4xl mx-auto mb-6 px-4 no-print flex justify-between items-center bg-white border border-slate-200 shadow-sm p-3.5 rounded-2xl">
        <span class="text-sm text-slate-500 font-medium">Purchase Return: <strong class="text-slate-800 font-mono">{{ $purchaseReturn->return_no }}</strong></span>
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('purchase-returns.show', $purchaseReturn->id) }}" class="inline-flex items-center gap-x-1.5 border border-slate-300 text-slate-700 text-xs font-semibold px-4 py-2 rounded-xl hover:bg-slate-50 transition-all">
                &larr; Back
            </a>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-xl border border-slate-200 rounded-2xl p-10 print:shadow-none print:border-0 print:rounded-none">
        ...
        <div class="flex justify-between items-start border-b border-slate-200 pb-8 mb-8">
            ...
            <div class="text-right">
                ...
                <div class="mt-3">
                    ...
                    @else
                        <span class="inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-700 border border-amber-200">PENDING</span>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-8 mb-8">
            <div>
                ...
                @if($purchaseReturn->supplier?->email) <p class="text-sm text-slate-500 mt-0.5">{{ $purchaseReturn->supplier->email }}</p> @endif
                @if($purchaseReturn->supplier?->phone) <p class="text-sm text-slate-500">{{ $purchaseReturn->supplier->phone }}</p> @endif
            </div>
            <div class="text-right">
                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Return Info</h3>
                <div class="mb-1"><span class="text-xs text-slate-400">Original PO:</span> <span class="font-mono font-bold text-slate-700">{{ $purchaseReturn->purchase->purchase_no ?? '-' }}</span></div>
                @if($purchaseReturn->reference_no)
                <div class="mb-1"><span class="text-xs text-slate-400">Reference:</span> <span class="font-semibold text-slate-700">{{ $purchaseReturn->reference_no }}</span></div>
                @endif
                <div><span class="text-xs text-slate-400">Created By:</span> <span class="font-semibold text-slate-700">{{ $purchaseReturn->user->name ?? '-' }}</span></div>
            </div>
        </div>
        ...
        <div class="flex justify-end mb-8">
            <div class="w-64 space-y-2 text-sm">
                <div class="flex justify-between items-center bg-slate-800 text-white px-3 py-2.5 rounded-xl">
                    ...
                    <span class="font-black text-lg">₹{{ number_format($purchaseReturn->grand_total, 2) }}</span>
                </div>
                <div class="flex justify-between text-violet-600 font-semibold pt-1">
                    <span>Refunded Amount</span>
                    <span>₹{{ number_format($purchaseReturn->refunded_amount, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-slate-200 pt-4 text-center text-xs text-slate-400">
            <p>This is a system-generated purchase return receipt from <strong>Antigravity POS</strong>.</p>
            <p class="mt-1">Created by: {{ $purchaseReturn->user->name ?? 'System' }} &bull; Printed on: {{ now()->format('d M Y, h:i A') }}</p>
        </div>
    </div>
</body>
</html>
