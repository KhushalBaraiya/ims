                                        ...
                                        @can('roles.delete')
                                            @if ($role->name !== 'super_admin')
                                                <form action="{{ route('roles.destroy', $role->id) }}" class="d-inline"
                                                    style="display:contents;" id="delete-form-{{ $role->id }}"
                                                    method="POST">
                                                    @csrf @method('DELETE')
        ...
        $(document).ready(function() {
            $('#rolesTable').DataTable({
                ...
                language: {
                    ...
                    emptyTable: '<div style="width:100%;text-align:center;padding:2.5rem 0;"><div class="text-muted" style="display:inline-flex;flex-direction:column;align-items:center;gap:8px;"><i class="bx bx-shield" style="font-size:3rem;opacity:.3;line-height:1;"></i>{{ __('messages.no_records') }}</div></div>',
                    paginate: {
