                ...
                templateResult: function(p) {
                    ...
                    return $(`
                        <div class="d-flex align-items-center gap-3 py-1">
                            ...
                            <div class="text-end flex-shrink-0">${badge}</div>
                        </div>`);
                },
            ...
            @if (!old('items'))
                @foreach ($adjustments as $adj)
                    ...
                    addProductRow({
                        ...
                        unit: "{{ $adj->product->unit_code ?? 'PCS' }}",
                        image: "{{ $adj->product->image ? asset('uploads/products/' . $adj->product->image) : 'https://placehold.co/40x40/e2e8f0/94a3b8?text=No+Image' }}",
                        qty: parseFloat("{{ $qty }}"),
                        type: "{{ $type }}"
                    });
                @endforeach
