...
@section('title', __('messages.edit_unit') . ' — ' . $unit->name)
...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-1">{{ __('messages.edit_unit') }}</h4>
    ...
    <form id="unitForm" method="POST" action="{{ route('units.update', $unit->id) }}" data-validate="true">
        @csrf @method('PUT')
        <div class="row g-4">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">
                                {{ __('messages.unit_name') }} <span class="text-danger">*</span>
                            </label>
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                value="{{ old('name', $unit->name) }}" placeholder="{{ __('messages.ph_unit_name') }}"
                                required>
                            @error('name')
                        ...
                        <div class="mb-0">
                            <label class="form-label fw-semibold">
                                {{ __('messages.short_name') }} <span class="text-danger">*</span>
                            </label>
                            <input type="text" name="short_name"
                                class="form-control @error('short_name') is-invalid @enderror"
                                value="{{ old('short_name', $unit->short_name) }}"
                                placeholder="{{ __('messages.ph_short_name') }}" required>
                    ...
                    <div class="card-body p-4">
                        <div class="mb-4">
                            ...
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <span id="statusLabel"
                                    class="fw-semibold {{ old('status', $unit->status) === 'active' ? 'text-success' : 'text-danger' }}">
                                    {{ old('status', $unit->status) === 'active' ? __('messages.active') : __('messages.inactive') }}
                                </span>
                            </div>
                        </div>
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('units.show', $unit->id) }}" class="btn btn-outline-secondary">
                    ...
                    <div class="card-body p-4">
                        <ul class="list-unstyled mb-0 small">
                            ...
                            <li class="d-flex justify-content-between py-2 border-bottom">
                                ...
                                <span>{{ $unit->created_at->format('d M Y') }}</span>
                            </li>
                            <li class="d-flex justify-content-between py-2">
                                <span class="text-muted fw-semibold">{{ __('messages.updated') }}</span>
                                <span>{{ $unit->updated_at->format('d M Y') }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
