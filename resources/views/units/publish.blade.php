    ...
    <div class="card-body p-4">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <div class="form-check form-switch mb-0">
                    <input class="form-check-input" type="checkbox" role="switch" id="statusToggle" name="status"
                        value="active" {{ old('status', $unit->status ?? 'active') == 'active' ? 'checked' : '' }}>
                </div>
                <span id="statusLabel"
                    class="fw-semibold {{ old('status', $unit->status ?? 'active') == 'active' ? 'text-success' : 'text-danger' }}">
                    {{ old('status', $unit->status ?? 'active') == 'active' ? __('messages.active') : __('messages.inactive') }}
                </span>
            </div>
        </div>
        <div class="d-grid gap-2">
            <button type="submit" class="btn btn-primary">
                ...
                {{ isset($unit) && $unit->exists ? __('messages.update_unit') : __('messages.save_unit') }}
            </button>
